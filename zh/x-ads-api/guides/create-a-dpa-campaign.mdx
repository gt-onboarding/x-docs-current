...
keywords: ["DPA 广告系列", "动态商品广告", "DPA", "创建 DPA 广告系列", "商品广告", "动态广告"]
---

以下示例假定你已经使用 twurl 完成了应用和用户的安装、配置和授权。twurl 是一个命令行工具，风格类似 cURL，可以方便地处理 X OAuth 身份验证。twurl 非常适合用于快速测试和调试 Ads API（以及 REST API）的功能。要查看请求和响应的完整头信息，请使用 -t 来跟踪调用，这大致相当于 cURL 的 -v 选项。
...
* 必填字段：资金工具、广告活动名称、预算（总额/每日）、开始时间和广告账户 ID
...
* 完成此步骤后，你就可以在 X 的 Ads Manager 中看到该广告活动。
...
* 必需参数：广告账户 ID 和 URL：
* URL 必须以 [https://twitter.com](https://twitter.com) 开头，后面可以追加任意 UTM 参数，例如：
  * `https://twitter.com?utm_source=%7B%7Bsite_source%7D%7D`
* 你也可以通过指定 creative&#95;type 来选择卡片的格式，可选值包括：&quot;CAROUSEL&quot;、&quot;SINGLE&quot;、&quot;COLLECTION&quot;。如果未指定，卡片将默认创建为轮播卡片。
...
* 必需参数：广告账户 ID、用户 ID、卡片 URI（来自第 3 步）、text、name 和 dynamic&#95;product&#95;ad
...
* 必需参数：广告账户 id、广告项目 id（来自步骤 2）以及帖子 id（步骤 4 中的 id&#95;str）
