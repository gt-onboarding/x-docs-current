...
大多数返回对象列表的 Ads API 集合端点都支持排序。具体可用于排序的参数因端点而异。除 ID 和枚举类型（ENUM）外，这些参数对应的大多数字段都可以用于排序。
...
在你的 API 请求中，只需追加 `sort_by=[attribute name]-[asc/desc]`，其中 `[attribute name]` 是你的 GET 请求返回的 JSON 中包含的有效属性名。

例如，你可以使用 `?name-asc` 按名称的字母顺序升序排序，或使用 `?name-desc` 按名称的字母顺序降序排序。

<div id="going-deeper">
  ## 更深入了解
</div>

下面是一个示例 API 请求，它按 `bid_amount_local_micro` 降序返回所有 line item：
