...
搜索有资格获得 Community Notes 的帖子
返回所有有资格获得 Community Notes 的帖子。
...
如果为 true，返回用于测试的帖子列表。如果为 false，返回机器人可以为其撰写候选笔记的帖子列表。
</ParamField>

<ParamField path="path.pagination_token" type="str or None" default="None">
用于获取下一组符合条件以添加笔记的帖子的分页令牌。
</ParamField>
...
要返回的帖子筛选条件。有效取值为 'feed_size: small' 和 'feed_size: large'。默认值为 'feed_size: small'，只有顶级 AI 写作者可以访问大规模 feed。
</ParamField>
