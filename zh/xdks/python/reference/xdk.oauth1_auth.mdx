...
该模块为安全授权流程提供 OAuth 1.0a 身份验证功能。包括请求令牌生成、授权 URL 的生成、访问令牌交换以及 OAuth 1.0a 签名生成。
