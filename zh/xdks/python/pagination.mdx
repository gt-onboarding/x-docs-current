...
X API 对返回多页结果的端点使用分页机制（例如时间线、搜索等）。每个 API 调用的响应都会包含一个 `meta` 对象，其中带有 `result_count`、`previous_token` 和 `next_token`。XDK 会负责基于 `next_token` 自动发起多次 API 调用，这样开发者只需指定期望获取的数据量，而无需手动多次调用。
SDK 通过以下方式简化了这一过程：

- **内置迭代器**：使用生成器函数无缝获取多页数据。
- **显式令牌处理**：在需要时可通过传递 `pagination_token` 实现灵活的手动控制。
- **最大结果数限制**：在每次调用中遵守 `max_results` 参数设置（受 API 上限约束，例如搜索最多 100 条）。

<div id="automatic-pagination-recommended">
  ## 自动分页（推荐）
</div>

在分页响应对象上使用 `iterate()` 方法，以惰性方式获取所有结果。
**示例：分页搜索**
...
如果你需要基于某些自定义逻辑（例如逐页处理）来控制结果，可以像下面所示那样继续使用 `next_token` 手动进行分页：
...
* 始终指定 `max_results` 以优化请求（默认值因端点而异）。
* 监控 `meta.result_count` 以便调试。
* 对于非常大的查询，考虑使用异步迭代以避免阻塞。
  如需使用 Python XDK 的详细代码示例，请参阅我们的 [代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/python)。
