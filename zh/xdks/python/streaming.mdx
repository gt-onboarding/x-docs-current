...
X API 通过诸如 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction) 等端点支持实时数据，实时传递符合条件的帖子。这需要建立一个持久的 HTTP 连接。
...
规则用于定义你要检索的特定数据的过滤条件（例如关键词、用户等）。你可以阅读[本指南](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)，了解更多关于如何构建规则的信息。
**添加规则**：
...
有关完整的规则语法，请参阅 [X Streaming Rules 文档](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)。
...
- **420 Enhance Your Calm**：已触发速率限制；请等待后重试。
- **No Data**：使用 `get_rules()` 检查规则；确认存在可匹配的帖子。
有关使用 Python XDK 的详细代码示例，请查看我们的[代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/python)。
如需更多示例和 API 参考，请查看内联 docstring（例如 `help(client.tweets.search_recent)`）或源码中的生成存根文件。你可以通过 [GitHub 仓库](https://github.com/xdevplatform/xdk/tree/main/xdk/python) 提交反馈。