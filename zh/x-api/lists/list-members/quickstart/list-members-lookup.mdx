...
title: 列表成员查询
sidebarTitle: 列表成员查询
keywords: ["列表成员查询快速入门", "列表成员查询", "获取列表成员快速入门", "列表成员资格查询"]
---

import { Button } from '/snippets/zh/button.mdx';

<Note>
  **请注意：** 本指南假定你已经完成了[快速开始概览](/zh/x-api/lists/list-members#getting-started-with-the-list-members-endpoint-group)中列出的所有前提条件。
</Note>
...
在将 X API v2 集合加载到 Postman 后，导航到 “List” 文件夹，选择其中的 “List members” 文件夹，然后选择 “Members lookup”。

**第二步：确定并指定你想要获取成员的 List**

你必须指定一个你希望获取成员的 List。你可以通过访问 x.com，点击某个 List，然后查看浏览器地址栏中的 URL 来找到该 List 的 ID。例如，下面这个 URL 中的 List ID 是 84839422。

https://x.com/i/lists/84839422

目标 ID 可以是任意有效的 List ID。在 Postman 中，导航到 “Params” 选项卡，将你的 ID 输入到 id 路径变量的 “Value” 列中。请务必不要在任何 ID 的前后包含空格。
...
如果你希望在 id、name 和 username 之外获取其他字段，你需要在请求中通过 [字段](/zh/x-api/fundamentals/fields) 和/或 [扩展字段](/zh/x-api/fundamentals/expansions) 参数来指定这些字段。

在本练习中，我们将从不同对象中请求三组额外字段：

* 主 user 对象中的额外 user.created&#95;at 字段。
* 使用 expansions 参数请求完整的 [Post object](/zh/x-api/fundamentals/data-dictionary#tweet)。
* 关联 Post 对象中的额外 tweet.created&#95;at 字段。

在 Postman 中，导航到 “Params” 选项卡，在 “Query Params” 表中添加以下键值对：
