...
在本节中，我们将学习如何通过命令行依次完成这些步骤。若要查看不同编程语言的示例代码，请访问我们的 [X API v2 示例代码 GitHub 仓库](https://github.com/xdevplatform/Twitter-API-v2-sample-code)。
...
首先，你需要创建一个合规任务，并指定你将上传的是帖子 ID 还是用户 ID（使用 `type` 参数）。另外，你还可以为该任务命名（使用 `name` 参数）。 

要为该请求授权，你需要使用 [App only](/zh/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token)。为此，请确保将下面的 `$APP_ACCESS_TOKEN` 替换为你在开发者门户中的 [X App](/zh/resources/fundamentals/developer-apps) 里生成的 [App Access Token](/zh/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token)。
...
请记录 upload&#95;url、download&#95;url 和 id 字段中的值；后续步骤中将会用到它们。

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### 步骤二：准备帖子 ID 或用户 ID 列表
</div>

创建一个包含帖子 ID 或用户 ID 的文本文件，其中每一行只包含一个帖子 ID 或用户 ID。文本文件的内容类似如下：
...
<Note>
  **注意**：上述文件可以只包含帖子 ID 或只包含用户 ID，不能混合两者。

  你可以通过一个 **PUT** 请求将文件直接上传到此 URL。请注意，该 URL 已经使用身份验证令牌签名，这意味着你不需要再次通过传递应用访问令牌来进行身份验证。请务必在请求中携带一个 Content-Type 头部，以表明你正在上传文本文件，并将下面的 $FILE&#95;LOCATION 替换为你的文件路径。
</Note>
...
较大的上传可能需要一些时间来处理。你可以在内容合规作业端点中指定你在第一步中获得的作业 ID，以查询作业状态。

同样，请务必将下方的 $APP&#95;ACCESS&#95;TOKEN 替换为你的应用访问令牌（App Access Token），并将 $ID 替换为你在第一步中获得的作业 ID。
...
当状态为 complete 时，表示结果已可以下载。注意：状态的其他可能取值包括 created、complete、in&#95;progress、failed 和 expired。

<div id="step-four-download-the-results">
  #### 步骤四：下载结果
</div>

在收到任务完成状态后，你可以从 `download_url` 字段中给出的 URL（同样在第一步中生成）下载合规结果。此 URL 已包含签名的身份验证令牌，这意味着你无需再次传递应用访问令牌来进行身份验证。
...
结果将包含一组 JSON 对象（每行一个对象）。每个对象将包含一个帖子或用户的 id、该帖子或用户的创建日期（便于按日期定位帖子）、需要执行的操作、合规操作的原因以及该操作的日期：
...
你的代码可以解析每一行 JSON 数据，定位帖子或用户 ID，并从数据集中删除具有这些 ID 的帖子和用户，以保持合规。对于你上传的某个 ID，如果没有对应的 JSON 对象，你可以认为该 ID 已合规。
