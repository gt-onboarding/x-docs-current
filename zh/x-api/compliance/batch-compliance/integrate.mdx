...
title: 集成指南
sidebarTitle: 集成指南
keywords: ["批量合规集成", "合规批量集成", "合规作业集成", "批量合规设置"]
---
...
在使用批量合规（Batch compliance）端点时，开发者可以批量上传大量 X 数据，并确定需要采取哪些操作，以确保其数据集能够反映用户意图以及 X 上内容的当前状态。当系统和网络连接稳定可靠时，将大量数据上传到远程服务器是一个相对简单的操作。但情况并非总是如此。某些环境可能会设置连接超时时间，在经过预设时长后，自动切断你的应用与上传服务器之间的连接；你也可能遇到连接问题，例如尝试通过 Wi‑Fi 从笔记本电脑上传大文件时。在这些情况下，将该文件拆分成较小的部分分次上传，而不是依赖单一的长时间连续连接，会更合适。

X 的批量合规端点依赖 Google Cloud Storage 来处理大型文件。这类存储针对多种应用场景进行了优化；Cloud Storage 支持一种用于管理大文件的技术，称为可恢复上传（resumable uploads）。

如果上传在任何阶段出现问题，Google Cloud Storage 都能够从中断的位置继续执行该操作。
...
首先，你需要创建一个合规作业，并通过 `type` 参数指定是上传 Post IDs 还是 user IDs。另需在请求体中加入 `resumable` 字段并将其设置为 `true`。请务必将下面的 `$APP&#95;ACCESS&#95;TOKEN` 替换为你的仅应用访问令牌（App-only Access Token）。
...
记下 `upload_url` 的值，后续步骤中会用到它。

<div id="step-two">
  #### 步骤二：
</div>

接下来，你需要启动可续传上传。为此，请向上一步中获取的 upload&#95;url 发起一次 **POST** 请求，并确保包含以下请求头：
...
如果此调用成功，您将收到 201 状态码。然后，在响应头中复制 `Location` 头的值，其形式大致如下：

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

然后，你可以按照[快速入门指南中的第二步及后续步骤](/zh/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)，将你的帖子 ID 或用户 ID 上传到此位置。

由于实现上较为复杂，断点续传功能最适合配合代码一起使用。本指南将使用 Node.js 搭配 [needle 请求库](https://www.npmjs.com/package/needle)。

<div id="install-the-dependencies">
  ### 安装依赖
</div>

在继续之前，你需要先安装好 Node.js 环境；你可以从 Node.js 官方网站下载安装程序。安装完成后，Node.js 会包含一个名为 npm 的工具；通过运行以下命令确认 Node 和 npm 都已正确安装，并确保该命令不会报错。

`$ npm -v
6.4.1`

出现类似这样的版本号表示你的环境已就绪（注意你的版本号可能不同）。我们将使用 npm 来安装上传库。运行以下命令：
...
创建新作业时，将 `resumable` 参数设置为 `true`，以便获取一个支持断点续传的上传目标地址。在响应负载中，你会收到一个 `upload&#95;url` 值。
...
默认情况下，该库会在接收到一个上传位置（称为 bucket）以及你希望上传的文件名后，创建一个新的上传目标。由于批量合规端点会自行创建上传目标，因此我们需要告知该库，我们已经有一个可以接受上传的位置。

我们需要将这个值连同包含要上传数据的文件的文件名一起传递给上传库。创建一个文件，并将其命名为 ***twitter-upload.js***。添加以下代码：
...
将文件保存在最合适的位置。接下来，在命令行中运行该脚本，并传入两个参数：

1. 第一个参数是你要上传的文件路径（该文件包含 Post 或 User ID）。
2. 第二个参数是我们从合规端点响应中获得的上传 URL。

请确保使用双引号将 URL 括起来；如果文件名中包含空格或其他字符，也同样要用双引号括起来：
...
你可以在任何时候通过按下 Ctrl + C 或关闭命令行来暂停上传。稍后再次执行相同命令时，你可以从中断的位置继续上传。文件完全上传后，你会看到以下消息：

`Upload complete`

此时，你就可以使用 [compliance status endpoint](/zh/x-api/compliance/get-compliance-job) 来检查合规任务的状态，并在完成后下载合规结果。
