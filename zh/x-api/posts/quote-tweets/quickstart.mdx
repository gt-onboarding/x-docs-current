...
title: 快速入门
sidebarTitle: 快速入门
keywords: ["引用帖子快速入门", "引用帖子教程", "引用帖子指南", "引用帖子示例", "引用转帖"]
---
...
本快速入门指南将帮助你通过 [Postman](/zh/tutorials/postman-getting-started)，在指定一组字段的情况下向引用帖子查询端点发送你的第一个请求。
...
有多种不同的工具、代码示例和库可以用来向此端点发起请求，但为了简化流程，我们这里将使用 Postman 工具。
...
在你将 X API v2 集合加载到 Postman 之后，导航到 timeline 文件夹，找到 “Quote Tweets by ID” 请求。

**第二步：为你的请求进行身份验证**

要正确地向 X API 发起请求，你需要验证自己是否拥有相应权限。对于这个端点，你必须使用 [OAuth 2.0 App-Only](/zh/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token)、[OAuth 2.0 Authorization Code with PKCE](resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2) 或 [OAuth 1.0a User Context](/zh/resources/fundamentals/authentication) 这三种身份验证方式之一来为请求进行身份验证。

为了简单起见，我们在本请求中将使用 OAuth 2.0 App-Only，但如果你希望请求私有[指标](/zh/x-api/fundamentals/metrics)或私有帖子，则需要使用其他身份验证方式之一。

你必须将自己的密钥和令牌（尤其是 [App Access Token](/zh/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token)，也称为 App-only Bearer Token）添加到 Postman 中。你可以在 Postman 右上角选择名为 “X API v2” 的环境，然后将你的密钥和令牌添加到 “initial value” 和 “current value” 字段中（点击环境下拉菜单旁边的眼睛图标即可编辑）。

如果你正确完成了这些操作，该变量会自动被填充到该请求的 Authorization 选项卡中。

**第三步：确定并指定你希望检索其 Quote Tweets 的帖子**

你必须在请求中通过传递 Post ID，指定你希望检索其 Quote Tweets 的帖子。

在本示例中，我们将使用的 Post ID 是 1409931481552543749。

在 Postman 中，进入 “Params” 选项卡，并将此 Post ID 填入 id 参数的 “Value” 列。
...
如果你在完成第三步后点击 “Send” 按钮，你将在响应中收到默认的 [Post object](/zh/x-api/fundamentals/data-dictionary#tweet) 字段：id、text 和 edit&#95;history&#95;tweet&#95;ids。

如果你希望接收其他字段，则必须在请求中使用 [field](/zh/x-api/fundamentals/fields) 参数和/或 [expansion](/zh/x-api/fundamentals/expansions) 参数来指定这些字段。

在本练习中，我们将从不同对象中请求三组额外的字段：

1. 主用户对象中额外的 tweet.created&#95;at 字段。
2. 返回帖子所关联作者的用户对象的默认字段：id、name 和 username
...
在 Postman 中，URL 字段中的路径参数 :id **不会**自动更新为你在 `id` 参数字段中输入的值，因此上述 URL 中显示的是 `:id`，而不是 1409931481552543749。
...
在第四步中，由于我们没有指定 `max_results` 参数，因此默认只能获得 10 条帖子。要访问后续页面的数据，我们将从上一页结果中获取 `next_token` 字段的值，并将该字符串作为 `pagination_token` 参数的值添加到 Postman 的 params 页面中，其余内容保持不变。 
...
完成以上设置后，你可以再次点击“Send”，此时就应该会收到下一页的结果。 
