...
keywords: ["integración de cumplimiento por lotes", "integración de cumplimiento en lotes", "integración de trabajos de cumplimiento", "configuración de cumplimiento por lotes"]
---

import { Button } from '/snippets/es/button.mdx';

<div id="working-with-resumable-uploads">
  ## Trabajar con subidas reanudables
</div>

Al usar los endpoints de Batch Compliance, los desarrolladores pueden subir por lotes grandes volúmenes de datos de X y determinar qué acción se necesita para garantizar que sus conjuntos de datos reflejen la intención del usuario y el estado actual del contenido en X. Subir grandes cantidades de datos a un servidor remoto es una operación relativamente sencilla cuando los sistemas y la conectividad son estables y confiables. Sin embargo, esto no siempre es así. Algunos entornos pueden imponer un tiempo de espera de conexión, lo que corta efectivamente la conexión entre tu aplicación y el servidor de carga después de un período de tiempo determinado; también puedes encontrarte con problemas de conexión, por ejemplo, cuando intentas subir un archivo grande desde tu computadora portátil a través de una conexión Wi‑Fi. En estas circunstancias, es preferible subir porciones más pequeñas de ese archivo a la vez, en lugar de mantener una única conexión continua.

Los endpoints de Batch Compliance de X se basan en Google Cloud Storage para procesar archivos grandes. Este tipo de almacenamiento está optimizado para varias aplicaciones; Cloud Storage admite una técnica para gestionar archivos grandes llamada subidas reanudables.

Si la subida falla en algún momento, Google Cloud Storage puede reanudar la operación desde donde se interrumpió.

<div id="creating-a-resumable-job">
  ### Crear una tarea reanudable
</div>
...
Primero, tendrás que crear una tarea de cumplimiento y especificar si subirás ids de Publicación o ids de usuario (usando el parámetro type). Además, añade resumable al cuerpo y establécelo en true. Asegúrate de reemplazar el $APP&#95;ACCESS&#95;TOKEN que aparece a continuación con tu Token de acceso de solo Aplicación.
...
A continuación, debes iniciar la carga reanudable. Para hacerlo, realiza una solicitud **POST** a `upload_url` del paso anterior y asegúrate de incluir los siguientes encabezados:
...
Si esta llamada se realiza correctamente, recibirás un código de estado 201. Luego, en los encabezados de la respuesta, copia el valor del encabezado Location, que tendrá un aspecto similar al siguiente:

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

A continuación, puedes subir los ID de Publicaciones o de usuarios a esta ubicación siguiendo a partir del paso dos de la [guía de inicio rápido](/es/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

Debido a su complejidad técnica, las cargas reanudables se utilizan mejor mediante código. Esta guía utilizará Node.js con la [biblioteca de peticiones needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Instala las dependencias
</div>

Antes de continuar, debes tener instalado un entorno de Node.js; puedes obtener Node.js desde su sitio web oficial. Una vez instalado, Node.js incluye una utilidad llamada npm; asegúrate de que tanto Node como npm estén instalados ejecutando el siguiente comando y verifica que no genere un error.
...
Al crear un nuevo trabajo, establece el parámetro `resumable` en `true` para obtener un destino que admita una carga reanudable. En el payload de respuesta, recibirás un valor `upload&#95;url`.
...
De forma predeterminada, la biblioteca creará un nuevo destino de carga a partir de una ubicación de carga (llamada bucket) y el nombre del archivo que deseas subir. Dado que los endpoints de cumplimiento por lotes (batch compliance) crean su propio destino, tendremos que indicarle a la biblioteca que ya contamos con una ubicación preparada para aceptar nuestra carga.

Debemos pasar este valor a la biblioteca de carga, junto con el nombre del archivo que contiene los datos que se van a subir. Crea un archivo y asígnale el nombre ***twitter-upload.js***. Agrega el siguiente código:
...
1. El primero será la ubicación del archivo (con los IDs de Publicaciones o Usuarios) que deseas cargar.
2. El segundo será la URL de carga que obtuviste en la respuesta del endpoint de cumplimiento.

Asegúrate de que la URL esté entre comillas dobles y haz lo mismo con el nombre de tu archivo si contiene espacios u otros caracteres:
...
Puedes pausar la carga en cualquier momento pulsando Ctrl + C o cerrando la ventana de la línea de comandos. Podrás reanudar la carga desde donde la dejaste cuando ejecutes el mismo comando más tarde. Una vez que el archivo se haya cargado por completo, verás el siguiente mensaje:

`Upload complete`

En este punto, podrás usar el [endpoint de estado de cumplimiento](/es/x-api/compliance/get-compliance-job) para comprobar el estado de tu trabajo de cumplimiento y podrás descargar el resultado de la tarea de cumplimiento cuando esté completo.
