...
keywords: ["inicio rápido de cumplimiento por lotes", "inicio rápido de cumplimiento en lotes", "inicio rápido de trabajos de cumplimiento", "tutorial de cumplimiento por lotes"]
---
...
Trabajar con los endpoints de cumplimiento por lotes generalmente consta de 5 pasos:

1. Crear un job de cumplimiento
2. Preparar la lista de id de Publicación o id de usuario
3. Cargar la lista de id de Publicación o id de usuario
4. Comprobar el estado del job de cumplimiento
5. Descargar los resultados

En esta sección aprenderemos cómo realizar cada uno de estos pasos usando la línea de comandos. Si deseas ver código de ejemplo en diferentes lenguajes de programación, visita nuestro [repositorio de código de ejemplo de X API v2 en GitHub](https://github.com/xdevplatform/Twitter-API-v2-sample-code).
...
Primero, tendrás que crear un trabajo de cumplimiento y especificar si vas a subir IDs de Publicaciones o IDs de usuarios (usando el parámetro type). Opcionalmente, también puedes darle un nombre a tu trabajo (usando el parámetro name). 

Para autorizar esta solicitud, tendrás que usar [App only](/es/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token). Para hacerlo, asegúrate de reemplazar el $APP&#95;ACCESS&#95;TOKEN que aparece abajo con tu [App Access Token](/es/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token), que puedes generar en tu [X App](/es/resources/fundamentals/developer-apps) en el portal para desarrolladores.
...
Si tu llamada a la API se realiza correctamente, obtendrás una respuesta similar a la siguiente:
...
Anota los valores de los campos upload&#95;url, download&#95;url e id; los necesitarás en los próximos pasos.

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### Paso dos: Prepara una lista de IDs de Publicación o IDs de usuario
</div>

Crea un archivo de texto con IDs de Publicación o IDs de usuario, donde cada línea contenga un ID de Publicación o un ID de usuario. El contenido del archivo de texto puede verse así:
...
<Note>
  **Nota**: El archivo anterior puede contener únicamente IDs de Publicación o únicamente IDs de usuario; no puede ser una mezcla de ambos.

  Puedes subir tu archivo directamente a esta URL mediante una solicitud **PUT**. Ten en cuenta que la URL ya está firmada con un token de autenticación, lo que significa que no necesitas autenticarte pasando de nuevo tu App Access Token. Asegúrate de incluir un encabezado Content-Type para indicar que estás subiendo un archivo de texto y reemplaza $FILE&#95;LOCATION a continuación por la ruta a tu archivo.
</Note>
...
Un código de estado 200 indica que la carga se realizó correctamente.

<div id="step-three-optional-check-your-job-status">
  #### Paso tres (opcional): Comprueba el estado de tu trabajo
</div>

Las cargas grandes pueden tardar algún tiempo en procesarse. Puedes solicitar una actualización del estado en el endpoint de trabajos de cumplimiento de contenido especificando el id del trabajo que recibiste en el primer paso.

Nuevamente, asegúrate de reemplazar el $APP&#95;ACCESS&#95;TOKEN que aparece a continuación por tu Token de acceso de la Aplicación y reemplazar el $ID por el id de tu trabajo del primer paso.
...
Un estado completo significa que los resultados están listos para que los descargues. Nota: Los otros valores posibles del estado son: created, complete, in&#95;progress, failed y expired.

<div id="step-four-download-the-results">
  #### Paso cuatro: Descarga los resultados
</div>

Cuando recibas la confirmación de que un job se ha completado, podrás descargar los resultados de cumplimiento desde la URL indicada en el campo download&#95;url (también generada en el primer paso). Esta URL ya está firmada con un token de autenticación, lo que significa que no necesitarás volver a autenticarte enviando tu App Access Token.
...
El resultado contendrá un conjunto de objetos JSON (un objeto por línea). Cada objeto contendrá una id de Publicación o de usuario, la fecha de creación de la Publicación o del usuario (útil para localizar Publicaciones por fecha), la acción necesaria, el motivo de la acción de cumplimiento y la fecha de dicha acción:
...
Tu código puede analizar cada línea JSON para localizar la Publicación o el id de usuario y eliminar las Publicaciones y los usuarios con esos id de tu conjunto de datos para mantenerte en cumplimiento. Si no hay un objeto JSON correspondiente para un id que cargaste, puedes asumir que ese id cumple con las políticas.
