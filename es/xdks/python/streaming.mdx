...
La X API admite datos en tiempo real a través de endpoints como el [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction), que entregan las Publicaciones coincidentes a medida que se publican. Esto requiere establecer una conexión HTTP persistente.
...
Las reglas definen filtros para los datos específicos que quieres obtener (por ejemplo, palabras clave, usuarios, etc.). Puedes obtener más información sobre cómo crear reglas en [esta guía](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule).
**Agregar reglas**:
...
- **420 Enhance Your Calm**: Límite de frecuencia alcanzado; espera y vuelve a intentarlo.
...
Para más ejemplos y la referencia de la API, revisa los *docstrings* en línea (por ejemplo, `help(client.tweets.search_recent)`) o las *stubs* generadas en el código fuente. Envía tus comentarios a través del [repositorio de GitHub](https://github.com/xdevplatform/xdk/tree/main/xdk/python).