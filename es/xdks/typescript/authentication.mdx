...
(async (): Promise<void> => {
  ...
  const authCode: string = 'code-from-callback-url'; // Sustituye por el code real del callback de OAuth
...
(async () => {
  ...
  // Después de la autorización, el usuario es redirigido con un parámetro code
  // Extrae el code de la URL de callback (por ejemplo, de los parámetros de consulta)
  const authCode = 'code-from-callback-url'; // Sustituye por el code real del callback de OAuth
...
<Info>
  Para obtener ejemplos de código detallados que usan el XDK de JavaScript/TypeScript, consulta nuestro [repositorio de ejemplos de código en GitHub](https://github.com/xdevplatform/samples/tree/main/javascript).
</Info>
