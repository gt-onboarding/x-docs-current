...
keywords: ["flujo OAuth de tres patas", "tokens de acceso de usuario", "flujo OAuth", "obtener tokens de acceso", "flujo OAuth 1.0a", "generación de tokens de acceso"]
---

import { Button } from "/snippets/es/button.mdx";

<div id="obtaining-access-tokens-using-3-legged-oauth-flow">
  ### Obtención de tokens de acceso usando el flujo OAuth de tres patas
</div>
...
A grandes rasgos, el proceso de OAuth con 3 participantes (3-legged OAuth) hará lo siguiente:
...
En la guía a continuación, verás distintos términos que se refieren a lo mismo.
...
* Token de acceso === Token === `oauth_token` resultante
* Secreto del token de acceso === Token Secret === `oauth_token_secret` resultante
...
Haz que el usuario se autentique y envíe a la aplicación cliente un request token.
 
...
Tras una autenticación correcta, tu `callback_url` recibirá una solicitud que contiene los parámetros `oauth_token` y `oauth_verifier`. Tu aplicación debe verificar que el token coincida con el request token recibido en el paso 1.
 
...
Para transformar el request token en un access token utilizable, tu aplicación debe realizar una solicitud al endpoint [POST oauth/access&#95;token](/es/resources/fundamentals/authentication/api-reference#post-oauth-access-token), que contenga el valor `oauth_verifier` obtenido en el paso 2. El request token también se pasa en la parte `oauth_token` del encabezado, pero esto se habrá agregado mediante el proceso de firma.
 
...
Ahora que has obtenido los tokens de acceso de usuario, puedes usarlos para acceder a ciertas API, como [POST statuses/update](/es/x-api/posts/manage-tweets/introduction) para crear Publicaciones en nombre de los usuarios.
 
...
El flujo estándar es basado en la web y utiliza el flujo de autorización OAuth de 3 pasos (3-legged). Las capturas de pantalla descritas aquí forman parte de un ejemplo cuyo código fuente puedes consultar en [https://github.com/xdevplatform/twauth-web](https://github.com/xdevplatform/twauth-web).

En algún momento del flujo de tu aplicación, querrás redirigir a X para autorizar tu aplicación.
...
Una vez que el usuario autorice tu aplicación, será redirigido a la URL de callback que proporcionaste cuando generaste el token de solicitud. Usarás esta redirección para obtener el token de acceso permanente de este usuario y almacenarlo localmente.
