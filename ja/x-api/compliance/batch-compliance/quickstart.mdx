...
keywords: ["バッチコンプライアンス クイックスタート", "コンプライアンス バッチ クイックスタート", "コンプライアンスジョブ クイックスタート", "バッチコンプライアンス チュートリアル"]
---

import { Button } from '/snippets/ja/button.mdx';

<div id="getting-started-with-the-batch-compliance-endpoints">
  ## バッチコンプライアンスエンドポイントの利用を開始する
</div>

バッチコンプライアンスエンドポイントを利用する際は、一般的に次の 5 つのステップが必要です。

1. コンプライアンスジョブを作成する
2. ポスト ID またはユーザー ID のリストを準備する
3. ポスト ID またはユーザー ID のリストをアップロードする
4. コンプライアンスジョブのステータスを確認する
5. 結果をダウンロードする

このセクションでは、コマンドラインを使ってこれらそれぞれのステップを実行する方法を説明します。さまざまなプログラミング言語でのサンプルコードを確認したい場合は、[X API v2 sample code GitHub repository](https://github.com/xdevplatform/Twitter-API-v2-sample-code) をご覧ください。

<div id="prerequisites">
  ### 前提条件
</div>

このガイドを完了するには、リクエストを認証するための [キーとトークン](/ja/resources/fundamentals/authentication) が必要です。これらのキーとトークンは、次の手順で生成できます。
...
まず、コンプライアンスジョブを作成し、ポストIDかユーザーIDのどちらをアップロードするかを、`type` パラメータを使って指定する必要があります。必要に応じて、`name` パラメータを使ってジョブ名を付けることもできます。 

このリクエストを認証するには、[App only](/ja/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token) を使用する必要があります。そのためには、以下の `$APP_ACCESS_TOKEN` を、developer portal 上の [X App](/ja/resources/fundamentals/developer-apps) で生成できる [App Access Token](/ja/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token) に置き換えてください。
...
upload&#95;url、download&#95;url、および id フィールドの値をメモしておいてください。これらは次の手順で必要になります。

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### ステップ2：ポストIDまたはユーザーIDのリストを準備する
</div>

ポストIDまたはユーザーIDを含むテキストファイルを作成し、各行に1つずつポストIDまたはユーザーIDを記載します。テキストファイルの内容は次のようになります。
...
<Note>
  **注**: 上記のファイルには、ポストIDかユーザーIDのどちらか一方のみを含める必要があり、両方を混在させることはできません。

  このURLに対して **PUT** リクエストを送信することで、ファイルを直接アップロードできます。URLはすでに認証トークンで署名されているため、App Access Token を再度渡して認証し直す必要はありません。テキストファイルをアップロードしていることを示すために Content-Type ヘッダーを必ず指定し、下記の $FILE&#95;LOCATION をご自身のファイルへのパスに置き換えてください。
</Note>
...
ステータスコード200は、アップロードが正常に完了したことを示します。

<div id="step-three-optional-check-your-job-status">
  #### ステップ3（任意）：ジョブのステータスを確認する
</div>

大容量のアップロードは処理に時間がかかる場合があります。最初のステップで受け取ったジョブ ID を指定して、コンテンツコンプライアンスジョブのエンドポイントからステータス更新をリクエストできます。

同様に、以下の $APP&#95;ACCESS&#95;TOKEN は利用しているアプリのアクセストークンに、$ID は最初のステップで取得したジョブ ID に置き換えてください。

`curl --request GET 'https://api.x.com/2/compliance/jobs/$ID' \ --header 'Authorization: Bearer $APP_ACCESS_TOKEN'`

レスポンスには、そのジョブに関する情報が含まれます。
...
ステータスが complete になっていれば、結果をダウンロードする準備ができていることを意味します。注：ステータスの値には、created、complete、in&#95;progress、failed、expired が使用されます。

<div id="step-four-download-the-results">
  #### ステップ4：結果をダウンロードする
</div>

ジョブの完了ステータスを受け取ったら、`download_url` フィールドに示されている URL（最初のステップで生成されたもの）からコンプライアンス結果をダウンロードできます。この URL にはすでに認証トークンが署名済みで含まれているため、アプリのアクセストークンを再度渡して認証する必要はありません。
...
結果には、JSON オブジェクトの集合（1 行につき 1 つのオブジェクト）が含まれます。各オブジェクトには、ポストまたはユーザーの id、ポストまたはユーザーの作成日時（ポストを日付ごとに整理して特定するのに有用）、必要なアクション、コンプライアンスアクションの理由、およびそのアクションが行われた日時が含まれます。
...
コードで各 JSON 行をパースしてポストまたはユーザーの ID を特定し、コンプライアンスを維持するために、その ID を持つポストおよびユーザーをデータセットから削除できます。アップロードした ID に対応する JSON オブジェクトが存在しない場合、その ID はコンプライアンスに適合しているとみなすことができます。

**注意**: すべてのコンプライアンスイベントに `redacted_at` フィールドが含まれているとは限りません。
