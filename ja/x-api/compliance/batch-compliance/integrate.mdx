...
title: 統合ガイド
sidebarTitle: 統合ガイド
keywords: ["バッチコンプライアンス統合", "コンプライアンスバッチ統合", "コンプライアンスジョブ統合", "バッチコンプライアンスセットアップ"]
---
...
Batch compliance エンドポイントを使用すると、開発者は大量の X データを一括アップロードし、そのデータセットがユーザーの意図と X 上のコンテンツの最新の状態を反映するために必要な対応を把握できます。システムや接続が安定して信頼できる状態であれば、大量のデータをリモートサーバーにアップロードすることは比較的単純な操作です。しかし、常にそうとは限りません。環境によっては接続のタイムアウトが設定されていて、一定時間経過後にアプリとアップロードサーバー間の接続が強制的に切断される場合があります。また、たとえばノート PC から Wi‑Fi 経由で大きなファイルをアップロードしようとしたときなどに、接続の問題に遭遇することもあります。このような状況では、1 本の連続した接続でアップロードするのではなく、そのファイルをより小さな部分に分割して順にアップロードできることが望ましくなります。

X の Batch compliance エンドポイントは、大きなファイルを処理するために Google Cloud Storage を利用しています。この種のストレージはさまざまな用途向けに最適化されており、Cloud Storage は「再開可能アップロード」と呼ばれる、大きなファイルを管理するための手法をサポートしています。

アップロード中にどこかで問題が発生した場合でも、Google Cloud Storage は中断された箇所から処理を再開できます。
...
まず、コンプライアンスジョブを作成し、`type` パラメータを使用して、ポスト ID をアップロードするかユーザー ID をアップロードするかを指定します。さらに、リクエストボディに `resumable` を追加し、その値を `true` に設定します。以下の `$APP&#95;ACCESS&#95;TOKEN` を、ご自身の App-only アクセストークンに必ず置き換えてください。
...
upload&#95;url の値をメモしておいてください。以降の手順で使用します。
...
この呼び出しが成功すると、レスポンスコード 201 が返されます。その後、レスポンスヘッダーに含まれる `location` ヘッダーの値をコピーします。この値は次のようになります。

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

その後、[クイックスタートガイド](/ja/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)のステップ2以降に従って、この場所にポストまたはユーザーのIDをアップロードできます。

技術的に複雑であるため、レジューム対応アップロードはコードと組み合わせて使用するのが最適です。このガイドでは、Node.js と [needle request library](https://www.npmjs.com/package/needle) を使用します。

<div id="install-the-dependencies">
  ### 依存関係をインストールする
</div>

先に進む前に、Node.js の実行環境をインストールしておく必要があります。Node.js は公式サイトから取得できます。インストールが完了すると、Node.js には npm というユーティリティが含まれています。次のコマンドを実行して Node と npm の両方がインストールされていることを確認し、エラーが発生しないことを確かめてください。

`$ npm -v
6.4.1`

このようなバージョン番号が表示されれば、環境の準備は完了です（表示されるバージョン番号は異なる場合があります）。npm を使ってアップロードライブラリをインストールします。次のコマンドを実行してください。
...
新しいジョブを作成するときは、再開可能アップロードに対応したアップロード先を取得できるよう、`resumable` パラメータを `true` に設定します。レスポンスのペイロードには、`upload_url` の値が含まれます。
...
デフォルトでは、このライブラリはアップロード先（バケットと呼ばれます）とアップロードしたいファイル名を受け取り、新しいアップロード先を作成します。バッチコンプライアンスエンドポイントは独自のアップロード先を作成するため、ライブラリに対して、すでにアップロード先が用意されていることを知らせる必要があります。 

このアップロード先を示す値に加えて、アップロードするデータを含むファイル名を、アップロード用ライブラリに渡す必要があります。ファイルを 1 つ作成し、名前を ***twitter-upload.js*** としてください。次のコードを追加します:
...
ファイルは、最も都合のよい場所に保存してください。次に、コマンドラインでスクリプトを実行し、2つのパラメータを渡します。

1. 1つ目は、アップロードしたいファイル（ポストまたはユーザー ID を含む）の場所です。
2. 2つ目は、コンプライアンスエンドポイントのレスポンスで受け取ったアップロード用 URL です。

URL は必ず二重引用符で囲み、ファイル名にスペースやその他の文字が含まれる場合も、同様に二重引用符で囲んでください。
...
Ctrl+C を押すか、コマンドラインを閉じることで、いつでもアップロードを一時停止できます。同じコマンドを後で再度実行すると、中断した箇所からアップロードを再開できます。ファイルのアップロードが完了すると、次のメッセージが表示されます。

`Upload complete`

この時点で、[コンプライアンスステータスエンドポイント](/ja/x-api/compliance/get-compliance-job)を使ってコンプライアンスジョブのステータスを確認したり、完了後にコンプライアンス結果をダウンロードしたりできるようになります。
