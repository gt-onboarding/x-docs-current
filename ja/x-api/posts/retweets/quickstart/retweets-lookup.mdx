---
title: リツイート ルックアップ
sidebarTitle: リツイート ルックアップ
keywords: ["リツイート ルックアップ", "リツイート ルックアップ クイックスタート", "リツイートの取得", "リツイート ルックアップ チュートリアル", "リツイート ルックアップ ガイド", "リツイート ルックアップの例"]
---
...
このクイックスタートガイドでは、[Postman](/ja/tutorials/postman-getting-started) を使って Retweets ルックアップエンドポイントへ最初のリクエストを送信するまでの手順を説明します。さまざまな言語でのサンプルコードをご覧になりたい場合は、GitHub 上の [X API v2 sample code](https://github.com/xdevplatformTwitter-API-v2-sample-code) リポジトリを参照してください。

<div id="prerequisites">
  #### 前提条件
</div>

このガイドを最後まで進めるには、リクエストを認証するための[キーとトークン](/ja/resources/fundamentals/authentication)一式が必要です。以下の手順に従って、これらのキーとトークンを発行できます。

* [開発者アカウントにサインアップ](https://developer.x.com/en/apply-for-access)し、承認を受けてください。
* 開発者ポータルで[プロジェクト](/ja/resources/fundamentals/projects)と、それに紐づく[開発者アプリ](/ja/resources/fundamentals/developer-apps)を作成します。
* アプリの「Keys and tokens」ページにアクセスして、必要な認証情報を発行します。すべての認証情報は、安全な場所に必ず保存してください。
...
1. プライマリユーザーオブジェクトに含まれる追加の user.created&#95;at フィールド。
2. 返されるユーザーに対して関連付けられたピン留めポストオブジェクトのデフォルトフィールドは、id と text です。
3. 関連するポストオブジェクト内に追加される tweet.created&#95;at フィールド。
...
すべての準備が整ったら、「Send」ボタンをクリックしてリクエストを送信します。すると、次の例のようなレスポンスが返ってきます。
