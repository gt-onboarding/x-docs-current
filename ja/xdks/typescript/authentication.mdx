...
TypeScript SDK は、さまざまなユースケースに対応する複数の認証方法をサポートしています。

<div id="bearer-token-app-only-auth">
  ## Bearer Token（アプリ単体認証）
</div>

読み取り専用の操作や公開データへのアクセスには、以下を使用します。
...
(async (): Promise<void> => {
  ...
  // ユーザーが authUrl にアクセスしてアプリを承認します
  // 承認後、ユーザーは code パラメータ付きで元のサイトにリダイレクトされます
  // コールバック URL（例: クエリパラメータ）から code を抽出します
  const authCode: string = 'code-from-callback-url'; // OAuth コールバックから取得した実際の code に置き換えてください
...
(async () => {
  ...
  // 承認後、ユーザーは code パラメータ付きで元のサイトにリダイレクトされます
  // コールバック URL（例: クエリパラメータ）から code を抽出します
  const authCode = 'code-from-callback-url'; // OAuth コールバックから取得した実際の code に置き換えてください
