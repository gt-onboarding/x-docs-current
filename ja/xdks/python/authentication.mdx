...
- **Bearer Token**: app-auth をサポートするエンドポイントへの読み取り専用アクセスに使用します（例: ポストの検索、ストリーミングエンドポイント）。
- **OAuth 2.0 PKCE**: スコープベースのユーザー承認アクセスのための安全な認証です（例: 認証済みユーザーのポストの non_public メトリクスの取得）。
- **OAuth 1.0a**: ユーザー固有の操作向けのレガシー認証です（例: ユーザーに代わってポストする、リストを管理する）。

[X Developer Portal](https://developer.x.com/en/portal/dashboard) から認証情報を取得してください。承認済みの開発者アカウントと、適切な権限（例: Read + Write）を持つアプリが必要です。
...
ユーザーコンテキストなしで読み取り専用の処理を実行する場合に使用します。
**手順**:
...
<div id="2-oauth-20-with-pkce-user-context">
  ### 2. OAuth 2.0 と PKCE を使う場合（ユーザーコンテキスト）
</div>

この例では、Proof Key for Code Exchange（PKCE）を用いた OAuth 2.0 の利用方法を示します。ユーザー固有のアクセス（例: ユーザーの代わりにポストする、ユーザー用にメディアをアップロードするなど）に使用します。
**手順**:

1. developer portal で、リダイレクト URI（例: `http://localhost:8080/callback`）付きでアプリを登録します。
...
**トークンの更新**（長期間有効なセッションでは SDK により自動的に実行されます）:
...
レガシーアプリケーションや、OAuth 1.0a 認証が必要な特定のユースケースの場合は、次の手順に従います。
**手順**:

1. Developer Portal で、API Key と API Secret を取得します。
2. すでにアクセストークンがある場合は、それらをそのまま使用します。ない場合は、OAuth 1.0a フローを実行してアクセストークンを取得します。
...
* SDK はトークンを検証し、失敗した場合は `xdk.AuthenticationError` をスローします。
  Python XDK を用いた詳細なコード例については、[code samples GitHub リポジトリ](https://github.com/xdevplatform/samples/tree/main/python) を参照してください。
