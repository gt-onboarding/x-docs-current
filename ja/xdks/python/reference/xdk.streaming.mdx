...
X API SDK 向けの堅牢なストリーミングユーティリティを提供します。

このモジュールは、自動再接続、指数バックオフ、包括的なエラー処理を備えたストリーミング接続の管理機能を提供します。クライアントは接続管理を気にすることなくストリーミングエンドポイントを利用でき、SDK がすべてのリカバリ処理を自動的に行います。

<div id="class-xdkstreamingstreamconfig">
  ### `class xdk.streaming.StreamConfig`
</div>

再試行動作を伴うストリーミング接続用の設定です。
...
分類情報付きのストリーミングエラーに対してスローされる例外です。
...
自動再接続と指数バックオフを使用して、エンドポイントからデータをストリーミングします。
この関数は、次のように接続管理全体を処理します：

- 切断時の自動再接続
- 再試行までの遅延に対するジッター付き指数バックオフ
- エラーを再試行可能かどうかと致命的かどうかで分類
