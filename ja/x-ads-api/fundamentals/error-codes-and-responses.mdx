...
成功したレスポンスは、200 番台の HTTP ステータスコードと、要求されたオブジェクトの取得・作成・更新・削除の結果に加えて、サーバーがリクエストをどのように解釈したかを示す JSON 形式のペイロードによって表されます。

成功したリクエストを送信した場合、レスポンスの一部として、送信したリクエストをそのまま反映した `request` ノードが含まれます。
...
JSON レスポンス内の `data` フィールドには、対象リソースに紐づく特定のオブジェクトが含まれます。レスポンスに 1 件以上の結果が含まれる可能性がある場合、`data` ノードの形式は JSON 配列になります。レスポンスとして 1 件の結果しか返されない場合は、JSON ハッシュとして返されます。まれに、通常であればコレクションを含むはずのレスポンスで、代わりにハッシュマップが含まれていることがあります。この場合、その単一のハッシュマップは、`type` フィールドで指定されているものと同じ型のオブジェクトであると見なしてください。

<div id="error-response-structure">
  ## エラー応答の構造
</div>

エラー応答は 200 番台以外の HTTP ステータスコードで返されます。通常は JSON レスポンスが返されますが、エラーによっては異なる種類のボディが返される場合があります。このようにレスポンス構造をパースできない状況では、HTTP コード本来の意味を優先して解釈してください。たとえば、HTTP 404 とともに HTML レスポンスが返される場合があります。このケースでは、コンテンツが見つからないと判断して問題ありません（HTTP 404 は「Not Found」を意味します）。

一般的なエラー応答は、成功時の応答と似た構造になっています。エラーの内容はレスポンスの `errors` ノードで伝えられます。`errors/code` ノードには、プログラム上で解決方法の判断に利用できる CAPS&#95;CASE 形式の定数エラーコードが含まれます。`errors/message` ノードには、エラーの英語での（通常は）人間が読める形の説明が含まれます。エラーの詳細をより細かく示すために、追加のフィールドが付与される場合もあります。
...
上記の例では、`start_time` パラメータに無効な値を指定してアナリティクスのエンドポイントにリクエストを送信しています。無効なパラメータを含むリクエストの `errors/code` は `INVALID_PARAMETER` です。
