...
이 가이드는 레거시 Enterprise Account Activity API에서 v2 Account Activity API로 마이그레이션하는 데 도움이 됩니다. 핵심 기능은 동일하게 유지되지만, X API v2와의 일관성을 맞추기 위해 엔드포인트 구조와 인증 방식이 업데이트되었습니다.

<div id="summary-of-changes">
  ### 변경 사항 요약
</div>

* **API Format**: 엔드포인트는 /1.1/account\_activity/ 대신 /2/account\_activity/ 기본 경로를 사용합니다.  
* **Webhook Management**: 웹훅 생성, 조회, 검증(CRC), 삭제는 [V2 Webhooks API](https://github.com/m-rosinsky/XWebhookTest/blob/main/app.py)로 처리됩니다.  
* **Authentication**: V2 엔드포인트 호출에는 다음 중 하나의 인증 방식이 필요합니다.  
  * 사용자 단위 작업(예: 사용자를 구독하는 작업)의 경우 **OAuthUser**(3-legged OAuth).  
  * 앱 단위 작업(예: 구독 조회/삭제)의 경우 **OAuth2 App Only**(Bearer Token).  
  * V1.1은 종종 OAuth 1.0a를 포함해 요구 사항이 일관되지 않았습니다.  
* **Endpoint Mapping**:

| V1.1 Endpoint | V2 해당 엔드포인트 / 동작 | 비고 |
| ----- | ----- | ----- |
...
| DELETE /1.1/account\_activity/webhooks/:webhook\_id/subscriptions/all | 사용 중단됨 / 직접 대응 엔드포인트 없음 | 개별 삭제를 수행하거나 DELETE /2/webhooks/:webhook\_id를 통해 웹훅을 제거합니다. |

* **Response Formats**: AAAPI 문서에서 성공 및 오류 응답 구조를 확인하세요.

마이그레이션 시, 새 기본 경로를 사용하도록 코드를 업데이트하고, 인증 플로를 조정하며, [V2 Webhooks API](https://docs.x.com/x-api/webhooks/introduction)를 사용해 웹훅을 관리해야 합니다.
