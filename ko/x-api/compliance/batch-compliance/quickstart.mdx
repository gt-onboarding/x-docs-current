...
배치 컴플라이언스 엔드포인트를 사용할 때는 일반적으로 다음 5단계를 거칩니다.
...
이 섹션에서는 명령줄을 사용하여 각 단계를 수행하는 방법을 살펴보겠습니다. 다양한 프로그래밍 언어로 작성된 샘플 코드를 보고 싶다면 [X API v2 샘플 코드 GitHub 리포지토리](https://github.com/xdevplatform/Twitter-API-v2-sample-code)를 방문하세요.

<div id="prerequisites">
  ### 사전 준비 사항
</div>

이 가이드를 완료하려면 요청을 인증하는 데 필요한 [키와 토큰](/ko/resources/fundamentals/authentication) 일련의 값이 있어야 합니다. 다음 단계에 따라 이러한 키와 토큰을 생성할 수 있습니다:

* [개발자 계정에 가입](https://developer.x.com/en/apply-for-access)하고 승인을 받으세요.
* developer portal에서 [프로젝트](/ko/resources/fundamentals/projects)를 생성하고 해당 [개발자 앱](/ko/resources/fundamentals/developer-apps)을 만드세요.
...
먼저 type 파라미터를 사용해 게시물 ID를 업로드할지, 사용자 ID를 업로드할지 지정하여 컴플라이언스 작업을 생성해야 합니다. 선택적으로 name 파라미터를 사용해 작업 이름을 지정할 수도 있습니다. 

이 요청을 인증하려면 [App only](/ko/resources/fundamentals/authentication#app-only-authentication-and-oauth-2-0-bearer-token) 방식을 사용해야 합니다. 이를 위해 아래의 $APP&#95;ACCESS&#95;TOKEN을 developer portal의 [X App](/ko/resources/fundamentals/developer-apps)에서 생성한 [App Access Token](/ko/resources/fundamentals/authentication#using-and-generating-an-app-only-bearer-token) 값으로 교체해야 합니다.
...
upload&#95;url, download&#95;url, 그리고 id 필드의 값을 메모해 두십시오. 이후 단계에서 이 값들이 필요합니다.

<div id="step-two-prepare-a-list-of-post-ids-or-user-ids">
  #### 2단계: 게시물 ID 또는 사용자 ID 목록 준비
</div>

각 줄에 게시물 ID 또는 사용자 ID가 하나씩 들어가도록, 게시물 ID 또는 사용자 ID 목록을 담은 텍스트 파일을 만드세요. 텍스트 파일의 내용은 다음과 비슷할 수 있습니다:
...
<Note>
  **참고**: 위 파일에는 게시물 ID 또는 사용자 ID 중 한 종류만 포함할 수 있으며, 두 가지를 섞어서 사용할 수 없습니다.

  이 URL로 **PUT** 요청을 보내 파일을 바로 업로드할 수 있습니다. 이 URL에는 이미 인증 토큰으로 서명되어 있으므로 앱 액세스 토큰을 다시 전달하여 인증할 필요가 없습니다. 텍스트 파일을 업로드하고 있음을 나타내기 위해 Content-Type 헤더를 반드시 포함하고, 아래의 $FILE&#95;LOCATION을 파일 경로로 교체하세요.
</Note>
...
200 상태 코드는 업로드가 성공적으로 완료되었음을 나타냅니다.
...
상태가 complete인 경우 결과를 다운로드할 수 있다는 뜻입니다. 참고: 상태 값은 created, complete, in&#95;progress, failed, expired 중 하나일 수 있습니다.

<div id="step-four-download-the-results">
  #### 4단계: 결과 다운로드
</div>

작업 완료 상태를 받으면 `download_url` 필드(1단계에서 생성된)에 표시된 URL에서 컴플라이언스 결과를 다운로드할 수 있습니다. 이 URL에는 이미 인증 토큰이 포함되어 있으므로, 앱 액세스 토큰을 다시 전달해 인증할 필요가 없습니다.
...
결과에는 JSON 객체 집합(각 줄에 하나의 객체)이 포함됩니다. 각 객체에는 게시물 또는 사용자 id, 게시물 또는 사용자의 생성 일시(날짜별로 정리된 게시물을 찾는 데 유용), 필요한 조치, 컴플라이언스 조치의 사유, 그리고 해당 날짜가 포함됩니다.
...
코드에서는 각 JSON 줄을 파싱하여 게시물 또는 사용자 id를 찾은 다음, 해당 id를 가진 게시물과 사용자를 데이터셋에서 삭제하여 컴플라이언스 요구사항을 충족할 수 있습니다. 업로드한 id에 해당하는 JSON 객체가 없다면, 해당 id는 컴플라이언스를 준수하는 것으로 간주할 수 있습니다.
