...
Batch compliance 엔드포인트를 사용할 때, 개발자는 대량의 X 데이터를 일괄 업로드하고, 사용자 의도와 X 상의 콘텐츠의 현재 상태를 데이터셋에 반영하기 위해 어떤 조치가 필요한지 파악할 수 있습니다. 시스템과 네트워크 연결이 안정적이고 신뢰할 수 있다면, 대량의 데이터를 원격 서버로 업로드하는 작업은 비교적 간단합니다. 그러나 항상 그런 것은 아닙니다. 일부 환경에서는 일정 시간이 지나면 연결 타임아웃을 적용해, 앱과 업로드 서버 사이의 연결을 강제로 끊을 수 있습니다. 또한 예를 들어 노트북에서 Wi‑Fi 연결을 통해 큰 파일을 업로드하려 할 때와 같이 연결 문제에 직면할 수도 있습니다. 이러한 상황에서는 하나의 연속된 단일 연결을 유지하기보다는, 파일을 더 작은 여러 부분으로 나누어 순차적으로 업로드하는 것이 바람직합니다.

X의 batch compliance 엔드포인트는 대용량 파일을 처리하기 위해 Google Cloud Storage에 의존합니다. 이 스토리지 서비스는 다양한 애플리케이션에 맞게 최적화되어 있으며, Cloud Storage는 재개 가능한 업로드(resumable uploads)라고 불리는 대용량 파일 관리 기법을 지원합니다.

업로드 도중에 문제가 발생하더라도 Google Cloud Storage는 중단된 지점부터 작업을 다시 이어서 수행할 수 있습니다.
...
먼저, 컴플라이언스 작업을 생성하고 `type` 파라미터를 사용해 게시물 ID를 업로드할지 사용자 ID를 업로드할지 지정해야 합니다. 또한 요청 본문에 `resumable`을 추가하고 값을 `true`로 설정합니다. 아래의 `$APP&#95;ACCESS&#95;TOKEN`을 앱 전용 액세스 토큰으로 반드시 교체하세요.
...
API 호출이 성공적으로 완료되면 다음과 같은 응답이 반환됩니다:
...
upload&#95;url의 값을 기록해 두세요. 이후 단계에서 필요합니다.

<div id="step-two">
  #### 2단계:
</div>

다음으로, 재개 가능한 업로드를 초기화해야 합니다. 이를 위해 이전 단계에서 받은 `upload_url`로 **POST** 요청을 보내고, 이때 다음 헤더를 포함해야 합니다:
...
이 호출이 성공하면 201 응답 코드를 받게 됩니다. 그런 다음 응답 헤더에서 `location` 헤더의 값을 복사합니다. 값은 대략 다음과 같은 형식입니다:

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

그런 다음 [빠른 시작 가이드](/ko/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)의 2단계부터 나머지 절차를 따라 이 위치에 게시물 또는 사용자 ID를 업로드할 수 있습니다.

기술적으로 복잡하기 때문에 재개 가능한 업로드는 코드를 함께 사용할 때 가장 효과적입니다. 이 가이드에서는 Node.js와 [needle 요청 라이브러리](https://www.npmjs.com/package/needle)를 사용합니다.

<div id="install-the-dependencies">
  ### 의존성 설치
</div>

계속 진행하기 전에 Node.js 환경이 설치되어 있어야 합니다. Node.js는 공식 웹사이트에서 내려받을 수 있습니다. 설치가 완료되면 Node.js에는 npm이라는 유틸리티가 함께 포함됩니다. 다음 명령을 실행해 Node와 npm이 모두 설치되어 있고, 오류가 발생하지 않는지 확인합니다.

`$ npm -v
6.4.1`

이와 비슷한 버전 번호가 표시되면 환경이 준비된 것입니다(표시되는 버전 번호는 다를 수 있습니다). 여기서는 npm을 사용해 업로드 라이브러리를 설치합니다. 다음 명령을 실행하세요:

`$ npm install -g needle`

이제 모든 준비가 완료되었습니다. 추가 설정은 필요하지 않습니다.

<div id="request-a-resumable-destination">
  #### 재개 가능한 대상 요청
</div>

새 작업을 생성할 때 재개 업로드를 지원하는 대상을 받으려면 `resumable` 매개변수를 `true`로 설정하세요. 응답 페이로드에서 upload&#95;url 값을 받게 됩니다.
...
기본적으로 이 라이브러리는 업로드 위치(버킷이라고 부름)와 업로드하려는 파일 이름을 입력받아, 새로운 업로드 대상 위치를 생성합니다. 하지만 배치 컴플라이언스(batch compliance) 엔드포인트는 자체적으로 대상 위치를 생성하므로, 이미 업로드를 받을 준비가 된 위치가 있다는 것을 라이브러리에 알려 주어야 합니다. 

업로드할 데이터가 들어 있는 파일의 이름과 함께 이 값을 업로드 라이브러리에 전달해야 합니다. 파일을 하나 만들고 이름을 ***twitter-upload.js***&#xB85C; 지정하세요. 다음 코드를 추가합니다:
...
파일을 적절한 위치에 저장합니다. 그런 다음 명령줄에서 스크립트를 실행하고 두 개의 매개변수를 전달합니다:

1. 첫 번째는 업로드하려는 파일의 위치입니다(게시물 또는 사용자 ID가 포함된 파일).
2. 두 번째는 컴플라이언스 엔드포인트 응답에서 받은 업로드 URL입니다.

URL은 큰따옴표로 감싸고, 파일 이름에 공백이나 기타 문자가 포함되어 있다면 파일 이름도 동일하게 큰따옴표로 감싸야 합니다:
...
Ctrl + C를 누르거나 명령줄 창을 닫아서 언제든지 업로드를 일시 중지할 수 있습니다. 나중에 동일한 명령을 다시 실행하면 중단한 지점부터 업로드를 재개할 수 있습니다. 파일 업로드가 완료되면 다음 메시지가 표시됩니다:

`Upload complete`

이 시점부터 [compliance status endpoint](/ko/x-api/compliance/get-compliance-job)를 사용해 컴플라이언스 작업 상태를 확인할 수 있고, 완료되면 컴플라이언스 결과를 다운로드할 수 있습니다.
