...
TypeScript SDK는 다양한 사용 사례에 맞게 여러 인증 방법을 지원합니다.

<div id="bearer-token-app-only-auth">
  ## Bearer Token (앱 전용 인증)
</div>

읽기 전용 작업과 공개 데이터에 액세스할 때 사용:
...
(async (): Promise<void> => {
  ...
  const authCode: string = 'code-from-callback-url'; // OAuth 콜백에서 실제 code로 교체하세요
...
(async () => {
  ...
  // 승인 후, 사용자는 code 파라미터와 함께 다시 리디렉션됩니다
  // 콜백 URL에서 code를 추출합니다(예: 쿼리 파라미터에서)
  const authCode = 'code-from-callback-url'; // OAuth 콜백에서 실제 code로 교체하세요
...
<Info>
  JavaScript/TypeScript용 XDK를 사용한 보다 자세한 코드 예제는 [code samples GitHub repo](https://github.com/xdevplatform/samples/tree/main/javascript)를 참고하세요.
</Info>
