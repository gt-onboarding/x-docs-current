...
TypeScript SDK는 라이브 데이터 피드를 실시간으로 스트리밍할 수 있는 기능을 제공합니다.

<div id="basic-streaming">
  ## 기본 스트리밍
</div>

실시간 샘플 게시물 스트림에 연결합니다:
...
for await (const event of stream) {
  // 각 이벤트는 파싱된 JSON 라인입니다 (data/includes/matching_rules)
  console.log(event);
}
```
...
<Info>
  JavaScript/TypeScript XDK를 사용한 자세한 코드 예제는 [코드 샘플 GitHub 리포지토리](https://github.com/xdevplatform/samples/tree/main/javascript)를 살펴보세요.
</Info>
