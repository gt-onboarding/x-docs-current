...
X API는 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction)와 같은 엔드포인트를 통해 실시간 데이터를 지원하며, 발생하는 즉시 조건에 맞는 게시물을 전달합니다. 이를 위해서는 지속적인 HTTP 연결을 맺어야 합니다.
...
규칙은 키워드, 사용자 등과 같은 조건을 기준으로 어떤 데이터를 수집할지에 대한 필터를 정의합니다. 규칙을 구성하는 방법은 [이 가이드](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)를 참고하세요.
**규칙 추가**:
...
자세한 규칙 구문은 [X Streaming Rules 문서](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)를 참고하세요.
...
- **403 Forbidden**: 인증 정보가 잘못됐거나 권한이 부족합니다.
- **420 Enhance Your Calm**: rate limit에 걸렸습니다. 잠시 기다렸다가 다시 시도하세요.
- **No Data**: `get_rules()`로 규칙을 확인하고, 일치하는 게시물이 존재하는지 확인하세요.
Python XDK를 사용하는 자세한 코드 예시는 [코드 샘플 GitHub 저장소](https://github.com/xdevplatform/samples/tree/main/python)를 참조하세요.
더 많은 예제와 API 레퍼런스는 인라인 docstring(예: `help(client.tweets.search_recent)`) 또는 소스에 포함된 자동 생성 스텁을 참고하세요. 피드백은 [GitHub 저장소](https://github.com/xdevplatform/xdk/tree/main/xdk/python)를 통해 보내주세요.