...
이 모듈은 안전한 인가 플로우를 위한 OAuth 1.0a 인증 기능을 제공합니다. 요청 토큰 생성, 승인 URL 생성, 액세스 토큰 교환, OAuth 1 서명 생성 기능을 포함합니다.

<div id="class-xdkoauth1_authoauth1">
  ### `class xdk.oauth1_auth.OAuth1`
</div>

X API에 대한 OAuth1 인증 핸들러입니다.
...
콜백 또는 PIN을 통해 전달된 OAuth verifier입니다.
</ParamField>
...
“Log in with X” 로그인 흐름을 사용할지 여부입니다.
</ParamField>
...
:returns: oauth_token 및 oauth_token_secret이 포함된 요청 토큰을 반환합니다.
...
<ParamField path="path.login_with_x" type="bool" default="False">
“Log in with X” 플로우를 사용할지 여부를 지정합니다.
</ParamField>
