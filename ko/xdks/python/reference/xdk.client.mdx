...
모든 하위 클라이언트를 조정하고, 인증과 세션 관리,
OAuth2 PKCE 흐름을 처리합니다. 모든 기능은 OpenAPI 명세를 기반으로 생성됩니다.
...
X API용 클라이언트 ID입니다(OAuth2 PKCE 플로우에 필요).
</ParamField>
...
OAuth2 인가용 리디렉션 URI입니다.
</ParamField>

<ParamField path="path.token" type="Dict[str, Any] or None" default="None">
기존 OAuth2 토큰 딕셔너리(있는 경우)입니다. 제공되면 access_token이 추출됩니다.
</ParamField>
...
인가 코드를 토큰으로 교환합니다(TypeScript API와 동일한 동작).
...
콜백으로부터 전달된 authorization 코드입니다.
</ParamField>
...
OAuth 2 PKCE 플로에서 사용할 인가 URL을 가져옵니다.
