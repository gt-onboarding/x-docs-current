...
북마크 삭제
인증된 사용자의 북마크에서 지정한 id에 해당하는 게시물을 제거합니다.
...
사용자 팔로우
인증된 사용자가 특정 사용자의 id를 사용해 해당 사용자를 팔로우하도록 합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
대상 사용자에 대한 팔로우를 요청하는 인증된 소스 사용자의 id입니다.
</ParamField>
...
차단 목록 조회
지정된 User ID가 차단한 사용자 목록을 조회합니다.
...
폴더 ID로 북마크 가져오기
인증된 사용자의 해당 ID를 가진 특정 북마크 폴더에 포함된 게시물을 가져옵니다.
...
id로 사용자 가져오기
지정된 사용자의 id로 해당 사용자의 세부 정보를 조회합니다.
...
쉼표(,)로 구분된 User ID 목록입니다. 최대 100개의 ID를 지정할 수 있습니다.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
표시할 사용자 필드의 쉼표(,)로 구분된 목록입니다.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
확장할 필드의 쉼표(,)로 구분된 목록입니다.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
표시할 트윗 필드의 쉼표(,)로 구분된 목록입니다.
</ParamField>
...
username으로 User 조회
username으로 특정 User의 세부 정보를 조회합니다.
...
username으로 사용자 가져오기
username으로 여러 사용자의 세부 정보를 가져옵니다.
...
팔로우 중인 리스트 가져오기
특정 사용자의 ID를 사용해, 해당 사용자가 팔로우 중인 리스트를 조회합니다.
...
팔로잉 가져오기
특정 User의 id를 사용해 해당 User가 팔로잉 중인 User들의 리스트를 가져옵니다.
...
멘션 조회
특정 사용자의 ID를 기준으로, 해당 사용자를 언급한 게시물 목록을 가져옵니다.
...
<ParamField path="path.pagination_token" type="Any or None" default="None">
다음 페이지의 결과를 가져올 때 사용하는 매개변수입니다.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물이 제공될 가장 이른 UTC 타임스탬프입니다. since_id 매개변수가 함께 지정된 경우 since_id가 우선합니다.
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물이 제공될 가장 늦은 UTC 타임스탬프입니다. until_id 매개변수가 함께 지정된 경우 until_id가 우선합니다.
</ParamField>
...
소유한 Lists 가져오기
특정 사용자의 id를 사용해 해당 사용자가 소유한 Lists의 목록을 조회합니다.
...
<ParamField path="path.since_id" type="Any or None" default="None">
결과 집합에 포함될 최소 게시물 ID입니다. 이 파라미터와 start_time이 모두 지정된 경우, 이 파라미터가 우선 적용됩니다.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
결과 집합에 포함될 최대 게시물 ID입니다. 이 파라미터와 end_time이 모두 지정된 경우, 이 파라미터가 우선 적용됩니다.
</ParamField>
...
<ParamField path="path.pagination_token" type="Any or None" default="None">
다음 "페이지" 결과를 가져오는 데 사용되는 파라미터입니다.
</ParamField>
...
<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 게시물을 제공하기 위한 가장 이른 UTC 타임스탬프입니다. since_id 파라미터도 지정된 경우, since_id가 우선 적용됩니다.
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 게시물을 제공하기 위한 가장 늦은 UTC 타임스탬프입니다. until_id 파라미터도 지정된 경우, until_id가 우선 적용됩니다.
</ParamField>
...
<ParamField path="path.poll_fields" type="List or None" default="None">
표시할 설문조사(Poll) 필드를 쉼표로 구분한 목록입니다.
</ParamField>
...
내 게시물의 리포스트 가져오기
인증된 사용자의 콘텐츠를 리포스트한 게시물 목록을 가져옵니다.
...
<ParamField path="path.pagination_token" type="Any or None" default="None">
이 매개변수는 다음 “페이지” 결과를 가져오는 데 사용합니다.
</ParamField>
...
타임라인 가져오기
인증된 사용자의 타임라인에서 게시물을 최신순으로 정렬한 리스트를 반환합니다.
...
<ParamField path="path.id" type="Any">
최신순 타임라인 게시물을 나열할 인증된 원본 사용자의 ID입니다.
</ParamField>
...
인증된 사용자가 특정 게시물 id를 사용하여 해당 게시물에 좋아요를 표시합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
게시물에 좋아요를 요청하는 인증된 소스 User의 ID입니다.
</ParamField>
...
사용자 음소거
인증된 사용자가 특정 사용자를 해당 id로 음소거합니다.
...
리스트 고정
인증된 사용자가 지정된 ID를 사용해 특정 리스트를 고정하도록 합니다.
...
게시물 리포스트
인증된 사용자가 특정 id의 게시물을 리포스트합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
게시물을 Repost하도록 요청하는 인증된 원본 사용자의 ID입니다.
</ParamField>
...
이 매개변수는 다음 ‘페이지’의 결과를 가져오는 데 사용됩니다. 이 매개변수에 사용하는 값은 API 응답에서 그대로 가져와야 하며, 변경해서는 안 됩니다.
</ParamField>
...
DM 차단 해제
인증된 사용자가 특정 사용자의 id를 기준으로 해당 사용자와 주고받는 다이렉트 메시지(DM)를 차단 해제합니다.
...
리스트 팔로우 취소
인증된 사용자가 지정된 ID의 특정 리스트를 팔로우 취소하도록 합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
List 팔로우를 해제할 인증된 소스 사용자의 id입니다.
</ParamField>

<ParamField path="path.list_id" type="Any">
팔로우를 해제할 List의 id입니다.
</ParamField>
...
<ParamField path="path.id" type="Any">
게시물에 대한 좋아요 취소를 요청하는 인증된 소스 사용자의 ID입니다.
</ParamField>
...
사용자 음소거 해제
인증된 사용자가 지정한 id 를 가진 사용자의 음소거를 해제합니다.
...
게시물 다시 게시 취소
인증된 사용자가 지정된 ID의 특정 게시물에 대한 다시 게시를 취소하도록 합니다.
