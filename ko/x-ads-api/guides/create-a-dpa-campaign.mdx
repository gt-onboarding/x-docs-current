...
다음 예시는 twurl을 사용해 앱과 사용자의 설치, 구성, 승인을 모두 완료했다고 가정합니다. twurl은 cURL과 유사한 명령줄 도구로, X OAuth 인증을 깔끔하게 처리합니다. twurl은 Ads API(및 REST API) 기능을 빠르게 테스트하고 디버깅하는 데 매우 유용한 도구입니다. 요청과 응답의 전체 헤더를 확인하려면 `-t` 옵션을 사용해 호출을 추적(trace)하세요. 이는 cURL의 `-v` 옵션과 대략 동일합니다.
...
앞선 단계에서 조회한 account id를 사용해 GET accounts/:account&#95;id/funding&#95;instruments API를 호출합니다.
...
캠페인을 생성한 다음 해당 펀딩 인스트루먼트에 연결합니다.
캠페인의 시작 일시와 예산을 설정합니다.

* 필수 필드: 펀딩 인스트루먼트, 캠페인 이름, 예산(총액/일별), 시작 일시, Ads 계정 ID
...
* catalog&#95;id 및 product&#95;set&#95;id는 쇼핑 매니저에서 확인할 수 있습니다.
...
* 이 단계를 완료한 후에는 캠페인을 X의 Ads Manager에서 확인할 수 있습니다.
...
* URL은 반드시 [https://twitter.com](https://twitter.com)으로 시작해야 하며, 아래 예시처럼 뒤에 추가 UTM 파라미터를 붙일 수 있습니다:
  * `https://twitter.com?utm_source=%7B%7Bsite_source%7D%7D`
* 선택적으로 `creative_type`에 다음 값 중 하나를 지정하여 카드 형식을 선택할 수 있습니다: &quot;CAROUSEL&quot;, &quot;SINGLE&quot;, &quot;COLLECTION&quot;. 지정하지 않으면 카드는 기본적으로 캐러셀 카드로 생성됩니다.
...
* 필수 매개변수: 광고 계정 ID, 사용자 ID, 카드 URI(3단계에서), 텍스트, 이름 및 dynamic&#95;product&#95;ad
...
* 필수 매개변수: 광고 계정 id, 라인 아이템 id (2단계에서 가져온 값), 트윗 id (4단계의 id&#95;str)
...
광고주가 UI에서 카드를 보고 싶다면, UI에서도 해당 카드가 표시됩니다.
